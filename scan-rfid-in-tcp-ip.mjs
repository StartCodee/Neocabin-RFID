...
const READER_HOST = process.env.RFID_READER_HOST || "192.168.179.201";
const READER_PORT = Number(process.env.RFID_READER_PORT || 2022);

// Use shared state file with OUT scanner so zone transitions IN<->OUT stay consistent.
...
async function sendToServer(epc, zone, meta = {}) {
  ...
  try {
    ...
    if (!resp.ok) {
      ...
      console.warn(`❌ [${ZONE}] backend ${resp.status}: ${bodyText || resp.statusText}`);
      return false;
    }
    return true;
  } catch (e) {
...
function connectReader() {
  ...
  client.on("connect", () => {
    ...
    if (INVENTORY_POLL) {
      ...
      console.log(`📶 [${ZONE}] inventory poll ON (${INVENTORY_INTERVAL_MS}ms)`);
    }
  });
